...
from ingestion.embedder import get_embedder  # adjust import to match your actual embedder module
from dotenv import load_dotenv
from ingestion.schema import EXCLUDED_COLLECTIONS
logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

load_dotenv()
MONGO_URI = os.getenv("MONGODB_URI")
DATABASE_NAME = os.getenv("MONGODB_DATABASE", "Olivia-karp")

# Collections to skip during auto-discovery (system/irrelevant ones)
EXCLUDE_COLLECTIONS = ["users", "sessions", "chat_history", "usage"]


def run():
    loader = MongoDBLoader(MONGO_URI, DATABASE_NAME)

    # Step 1 — dynamically discover + load + flatten every collection
    collection_data = loader.load_all_collections(exclude=EXCLUDE_COLLECTIONS)
    loader.close()

    # Step 2 — chunk each collection's documents
    chunker = Chunker()
    chunked_data = chunker.chunk_all_collections(collection_data)

    # Step 3 — embed + build FAISS index per collection
    embedder = get_embedder()
    indexer = MongoDBVectorIndexer(embedder=embedder)
    indexer.build_all_indexes(chunked_data)
    indexer.save_all_indexes()

    logger.info("Ingestion complete.")
    logger.info(f"Indexed collections: {list(chunked_data.keys())}")
